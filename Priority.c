...
int main(){
    printf("\nPriority process scheduling (Non-Preemptive)\n");
    printf("\nHigher the priority of a process, the sooner it gets scheduled for execution.\n\n");
    ...
    int *priority=(int*)calloc(totalProcesses,sizeof(int));
    int *priorityCopy=(int*)calloc(totalProcesses,sizeof(int));
    for(int i=0;i<totalProcesses;i++){
        ...
        scanf("%d",&burstTime[i]);
        printf("Priority of process P%d : ",i);
        scanf("%d",&priority[i]);
        priorityCopy[i]=priority[i];
    }
    ...
    while(completedProcesses<totalProcesses){
        int maxPt=-1e9,idx=-1;
        for(int i=0;i<totalProcesses;i++){
            if(priorityCopy[i]>maxPt&&arrivalTime[i]<=time) {
                maxPt=priorityCopy[i];
                idx=i;
            }
        }
        if(idx==-1) {
        ...
        priorityCopy[idx]=-1e9;
        addTime(&ganttChartTime,time);  
        addTime(&ganttChartTime,time+burstTime[idx]);  
    ...
    for(int i=1;i<=ganttChartProcessTimeLen;i++){
        printf("%2d    ",ganttChartTime[i]);
        i++;
    }

    printf("\n\nThe scheduling for Priority Non-Preemptive:\n");
    printf("\nProcess Id | Arrival Time | Burst Time | Priority | Completion Time | Turnaround Time | Waiting Time\n");
    for(int i=0;i<totalProcesses;i++) printf("P%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n",pid[i],arrivalTime[i],burstTime[i],priority[i],completionTime[i],turnAroundTime[i],waitingTime[i]);
